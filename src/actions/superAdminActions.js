...
export const makeSuperAdmin = (id) => async (dispatch, getState) => {
  try {
    dispatch({
      type: SUPER_MAKE_SUPER_ADMIN_REQUEST,
    });
    ...
    const { data } = await axios.put(
      `${variables.backendLink}/api/super-admin/users/admin/${id}`,
      "",
      config
    );

    dispatch({
      type: SUPER_MAKE_SUPER_ADMIN_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: SUPER_MAKE_SUPER_ADMIN_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
...
export const demoteAdmin = (id) => async (dispatch, getState) => {
  try {
    dispatch({
      type: SUPER_DEMOTE_ADMIN_REQUEST,
    });
    ...
    const { data } = await axios.put(
      `${variables.backendLink}/api/super-admin/users/admin/${id}/remove`,
      "",
      config
    );

    dispatch({
      type: SUPER_DEMOTE_ADMIN_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: SUPER_DEMOTE_ADMIN_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
